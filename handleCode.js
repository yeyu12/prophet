...
function handleJsCode(jsCode, noHead) {
    ...
    ObjectExpression(path) {
      for (let i = 0; i < path.node.properties.length; i++) {
        if (path.node.properties[i].type != "ObjectMethod") {
          path.node.properties[i].value = t.CallExpression(t.Identifier("window.ting_hook"),
            [
  ...
  // 在这里要压缩js文件，否则可能会出现内存爆破的问题
  // JsCode = uglify.minify(JsCode).code

  // console.log("noHead:----------------", noHead)
  // if (noHead) {
  //   return uglify.minify(JsCode).code
  // } else {
  //   return uglify.minify(head_code + JsCode).code
  // }
